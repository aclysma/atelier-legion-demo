...
impl EditorDrawResource {
    ...
    pub fn add_line(&mut self, id: &str, debug_draw: &mut DebugDrawResource, p0: glm::Vec2, p1: glm::Vec2, mut color: glm::Vec4) {
    ...
    // Input here is world space
    pub fn add_circle_outline(&mut self, id: &str, debug_draw: &mut DebugDrawResource, center: glm::Vec2, radius: f32, mut color: glm::Vec4) {
        if self.closest_shape_to_mouse.id == id && self.closest_shape_to_mouse.distance_sq < MAX_MOUSE_INTERACT_DISTANCE_FROM_SHAPE_SQ {
            color = glm::vec4(1.0, 0.0, 0.0, 1.0);
        }
    ...
    fn get_closest_shape(&self, test_position: glm::Vec2, viewport: &ViewportResource) -> Option<ClosestShapeIndexDistance> {
        ...
        for i in 0..self.shapes.len() {
            ...
            let distance_sq = match &shape.shape {
                ...
                Shape::CircleOutline(circle) => {
                    ...
                    let scaled_position_on_outline = viewport.world_space_to_ui_space(position_on_outline);
    ...
    pub fn update(
        ...
        let closest_shape = self.get_closest_shape(to_glm(mouse_position), viewport);
        ...
        for mouse_button_index in 0..InputState::MOUSE_BUTTON_COUNT {
            ...
            if let Some(down_position) = input_state.mouse_button_went_down_position(mouse_button) {
                let down_position = to_glm(down_position);
            ...
            if let Some(current_drag_in_progress) = &self.shape_drag_in_progress[mouse_button_index] {
                ...
                if let Some(input_state_drag_in_progress) = &input_state.mouse_drag_in_progress(mouse_button) {
                    ...
                    let world_space_end_position = viewport.ui_space_to_world_space(to_glm(input_state_drag_in_progress.end_position));
                    ...
                    self.shape_drag_in_progress[mouse_button_index] = Some(EditorShapeDragState {
                        begin_position: to_glm(input_state_drag_in_progress.begin_position),
                        end_position: to_glm(input_state_drag_in_progress.end_position),
                        previous_frame_delta: to_glm(input_state_drag_in_progress.previous_frame_delta),
                        accumulated_frame_delta: to_glm(input_state_drag_in_progress.accumulated_frame_delta),
                    ...
                    let world_space_end_position = viewport.ui_space_to_world_space(to_glm(input_state_drag_just_finished.end_position));
                    let delta = world_space_end_position - (current_drag_in_progress.world_space_begin_position + current_drag_in_progress.world_space_accumulated_frame_delta);

                    self.shape_last_interacted = current_drag_in_progress.shape_id.clone();
                    self.shape_drag_just_finished[mouse_button_index] = Some(EditorShapeDragState {
                        begin_position: to_glm(input_state_drag_just_finished.begin_position),
                        end_position: to_glm(input_state_drag_just_finished.end_position),
                        previous_frame_delta: to_glm(input_state_drag_just_finished.previous_frame_delta),
                        accumulated_frame_delta: to_glm(input_state_drag_just_finished.accumulated_frame_delta),
                        world_space_begin_position: current_drag_in_progress.world_space_begin_position,
                        world_space_end_position,
                        world_space_previous_frame_delta: delta,
                        world_space_accumulated_frame_delta: delta + current_drag_in_progress.world_space_accumulated_frame_delta,
                ...
                if self.closest_shape_to_mouse.distance_sq < MAX_MOUSE_INTERACT_DISTANCE_FROM_SHAPE_SQ {
                    ...
                    if let Some(mouse_drag_in_progress) = input_state.mouse_drag_in_progress(mouse_button) {
                        ...
                        if let Some(down_on_shape) = &self.mouse_is_down_on_shape[mouse_button_index] {
                            if let Some(closest_shape) = self.get_closest_shape(to_glm(mouse_drag_in_progress.begin_position), viewport) {
                                let shape = &self.shapes[closest_shape.index];
                                if closest_shape.distance_sq < MAX_MOUSE_INTERACT_DISTANCE_FROM_SHAPE_SQ &&
                                    down_on_shape.shape_id == shape.id
                                {
                                    let world_space_begin_position = viewport.ui_space_to_world_space(to_glm(mouse_drag_in_progress.begin_position));
                                    let world_space_end_position = viewport.ui_space_to_world_space(to_glm(mouse_drag_in_progress.end_position));
                                    ...
                                    self.shape_drag_in_progress[mouse_button_index] = Some(EditorShapeDragState {
                                        begin_position: to_glm(mouse_drag_in_progress.begin_position),
                                        end_position: to_glm(mouse_drag_in_progress.end_position),
                                        previous_frame_delta: to_glm(mouse_drag_in_progress.previous_frame_delta),
                                        accumulated_frame_delta: to_glm(mouse_drag_in_progress.accumulated_frame_delta),
                        ...
                        let just_clicked_position = to_glm(just_clicked_position);
                        // check if we clicked a shape
                        if let Some(down_on_shape) = &self.mouse_is_down_on_shape[mouse_button_index] {
                            if let Some(closest_shape) = self.get_closest_shape(just_clicked_position, viewport) {
                                let shape = &self.shapes[closest_shape.index];
                                if closest_shape.distance_sq < MAX_MOUSE_INTERACT_DISTANCE_FROM_SHAPE_SQ &&
                                    down_on_shape.shape_id == shape.id
                                {
    ...
    pub fn is_shape_drag_in_progress(&self, mouse_button: MouseButton) -> bool {
        self.shape_drag_in_progress[InputState::mouse_button_to_index(mouse_button).unwrap()].is_some()
    }
    ...
    pub fn is_shape_drag_just_finished(&self, mouse_button: MouseButton) -> bool {
        self.shape_drag_just_finished[InputState::mouse_button_to_index(mouse_button).unwrap()].is_some()
    }
